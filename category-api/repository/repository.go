...
func GetAllCategories() ([]model.Category, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&category.ID, &category.Category, &category.CreatedAt, &category.UpdatedAt); err != nil {
			return nil, err
		}
...
func GetAllCategoriesWithDeleted() ([]model.Category, error) {
	...
	for rows.Next() {
		var category model.Category
		if err := rows.Scan(&category.ID, &category.Category, &category.CreatedAt, &category.UpdatedAt, &category.DeletedAt); err != nil {
			return nil, err
		}
...
func GetCategoryByID(id uint64) (*model.Category, error) {
	...
	var category model.Category
	if err := row.Scan(&category.ID, &category.Category, &category.CreatedAt, &category.UpdatedAt); err != nil {
...
func CreateCategory(category *model.Category) error {
	...
	category.UpdatedAt = &formattedTime

	query := "INSERT INTO categories (category, created_at, updated_at) VALUES (?, ?, ?)"
	result, err := config.DB.Exec(query, category.Category, category.CreatedAt, category.UpdatedAt)
	if err != nil {
		return err
	}
...
func UpdateCategory(category *model.Category) error {
	...
	_, err = config.DB.Exec(query, category.Category, category.UpdatedAt, category.ID)
	if err != nil {
		return err
	}
